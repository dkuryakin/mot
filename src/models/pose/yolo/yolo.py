...
def preprocess_image(
    image: np.ndarray,
    yolo_size=DEFAULT_YOLO_INPUT_SIZE,
    input_size=DEFAULT_IMAGE_INPUT_SIZE,
):
...
def postprocess_prediction(
    ...
    prediction = np.transpose(
        prediction, (0, 2, 1)
    )  # shape(1, 84, 6300) -> shape(1, 6300, 84)
...
class Prediction:
    def __init__(self, boxes: np.ndarray, keypoints: np.ndarray, img_shape: tuple):
        ...
        self._keypoints = Keypoints(
            keypoints[:, :, :2]
            if len(keypoints)
            else np.empty((0, *YOLO_KEYPOINTS_SHAPE)),
            img_shape,
        )
...
class YoloTritonClient:
    ...
    def inference(self, image: np.ndarray):
        ...
        inputs = [InferInput(YOLO_INPUT_NAME, image_array.shape, YOLO_PRECISION)]
        inputs[0].set_data_from_numpy(image_array)

        # Prepare output tensor
        outputs = [InferRequestedOutput(YOLO_OUTPUT_NAME)]
        ...
        pred_kpts = (
            pred[:, 6:].reshape(len(pred), *kpt_shape) if len(pred) else pred[:, 6:]
        )
...
if __name__ == "__main__":
    ...
    yolo = YoloTritonClient(
        triton_server_url="192.168.2.135:8001", model_name="yolo", model_version="1"
    )

    image = cv2.imread("pp_triton/tests/test.jpeg")
    ...
    cv2.imwrite("pp_triton/tests/out.jpeg", image)
